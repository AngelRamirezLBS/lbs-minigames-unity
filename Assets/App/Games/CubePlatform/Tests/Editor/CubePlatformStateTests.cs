...
{
    public sealed class CubePlatformStateTests
    {
        [Test] public void Box3_IsTheSemanticCorrectAnswer() => Assert.IsTrue(CubePlatformRule.IsCorrect("box3"));
        [Test] public void FirstTouchLocksUntilResolution()
        {
            var state = new SelectionGameState();
            Assert.IsFalse(state.Select("box1", CubePlatformRule.CorrectAnswer));
            Assert.IsFalse(state.Select("box3", CubePlatformRule.CorrectAnswer));
        ...
        {
            var state = new SelectionGameState();
            state.Select("box2", CubePlatformRule.CorrectAnswer);
        ...
        {
            var state = new SelectionGameState();
            Assert.IsTrue(state.Select("box3", CubePlatformRule.CorrectAnswer));
        ...
        {
            var state = new SelectionGameState();
            state.Select("box3", CubePlatformRule.CorrectAnswer);
