...
namespace Lbs.MiniGames.Games.ShapeAnalogy
{
    public sealed class ShapeAnalogyGame : MonoBehaviour, IAppScene
    {
        ...
        private void Build()
        {
            ...
            Button exit = UiFactory.CreateButton(board, "Exit", font, "", Color.clear); PixelRect(exit.GetComponent<RectTransform>(), new(110,150), new(170,170)); exit.GetComponentInChildren<Text>().gameObject.SetActive(false); Image exitImage = UiFactory.CreateImage(exit.transform as RectTransform, "ExitArtwork", Color.white); exitImage.sprite = exitIcon; exitImage.preserveAspect = true; AddArtworkShadow(exitImage, 2f, .14f); UiFactory.Stretch(exitImage.rectTransform, 0); exit.onClick.AddListener(ReturnToLobby);
