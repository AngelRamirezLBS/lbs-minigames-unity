...
This sequence delivers four short logic activities in a fixed order for the landscape, touch-first UI: **Shape Analogy → Clothes Selection → Object Selection → Make An Emoji Drag → Lobby**. Use this page as the contributor-level map; implementation references are included only where maintenance requires them.
...
| Make An Emoji Drag | `make.emoji.drag` | Build a happy emoji. | Drag the three emoji strips in any order; each left-side slot accepts only its matching strip and correct strips remain placed. | Completion, then Lobby |
...
- **BGM:** `shape.analogy`, `clothes.selection`, `object.selection`, and `make.emoji.drag` are the explicit logic-sequence BGM members. See `Assets/App/Navigation/LevelSequenceRoute.cs`.
...
- Game rules: `Assets/App/Games/ShapeAnalogy/Core/ShapeAnalogyRule.cs`, `Assets/App/Games/ClothesSelection/Core/ClothesSelectionRule.cs`, `Assets/App/Games/ObjectSelection/Core/ObjectSelectionRule.cs`, `Assets/App/Games/MakeAnEmojiDrag/Core/MakeAnEmojiDragRule.cs`
- Scenes: `Assets/App/Games/ShapeAnalogy/ShapeAnalogy.unity`, `Assets/App/Games/ClothesSelection/ClothesSelection.unity`, `Assets/App/Games/ObjectSelection/ObjectSelection.unity`, `Assets/App/Games/MakeAnEmojiDrag/MakeAnEmojiDrag.unity`
