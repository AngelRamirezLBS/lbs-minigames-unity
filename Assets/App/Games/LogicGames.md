...
This sequence delivers three short logic activities in a fixed order for the landscape, touch-first UI: **Shape Analogy → Clothes Selection → Object Selection → Lobby**. Use this page as the contributor-level map; implementation references are included only where maintenance requires them.
...
| Object Selection | `object.selection` | Find the object that does not belong. | Tap an answer card; the current round contains a tennis shoe among hats, and the tennis shoe is correct. | Completion, then Lobby |

## Shared behavior

- **Landscape and touch:** the games use the shared level chrome and touch-compatible UI; Shape Analogy is drag-and-drop, while the selection games use answer cards.
...
- **BGM:** `shape.analogy`, `clothes.selection`, and `object.selection` are the explicit logic-sequence BGM members. See `Assets/App/Navigation/LevelSequenceRoute.cs`.
...
- Game rules: `Assets/App/Games/ShapeAnalogy/Core/ShapeAnalogyRule.cs`, `Assets/App/Games/ClothesSelection/Core/ClothesSelectionRule.cs`, `Assets/App/Games/ObjectSelection/Core/ObjectSelectionRule.cs`
- Scenes: `Assets/App/Games/ShapeAnalogy/ShapeAnalogy.unity`, `Assets/App/Games/ClothesSelection/ClothesSelection.unity`, `Assets/App/Games/ObjectSelection/ObjectSelection.unity`
